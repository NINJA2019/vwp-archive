...
function fallbackResultPage(siteUrl, id, sentVideo, bottle, fbVideo) {
  const ogTitle = `#ObserverLink で「${fbVideo.title}」をおすすめされました！`;
  ...
  const fbYtId = extractYtId(fbVideo.url);
  const fbThumb = fbYtId ? `https://img.youtube.com/vi/${fbYtId}/mqdefault.jpg` : '';
  const fbMemberName = getMemberDisplay(fbVideo.member || '');
  const fbColor = getMemberColor(fbVideo.member || '');
  ...
  const fbCardHtml = `
    <div class="card">
      <div class="card-label received">RECOMMENDED</div>
      <div class="card-content">
        ${fbThumb ? `<img class="card-thumb" src="${escHtml(fbThumb)}" alt="">` : ''}
        <div>
          <div class="card-member" style="color:${fbColor}">${escHtml(fbMemberName)}</div>
          <div class="card-title">${escHtml(fbVideo.title || '')}</div>
        </div>
      </div>
      ${fbYtId ? `<a class="yt-btn" href="https://www.youtube.com/watch?v=${escHtml(fbYtId)}" target="_blank" rel="noopener">&#9654; YouTubeで聴く</a>` : ''}
    </div>`;
...
<div class="container">
  ...
  <div class="intro">
    <div class="intro-icon">✦</div>
    <div class="intro-title">Here's a recommendation</div>
    <div class="intro-sub">No bottles in the sea yet &mdash;<br>but we found something for you</div>
  </div>
  ${sentCardHtml}
  ${fbCardHtml}
  <a class="cta" href="${siteUrl}">Try Observer-Link</a>
